...
public enum SimpleDatabase {
    ...
    private SimpleDatabase() {
        ...
        ExchangeModel usdEur = new ExchangeModel(usd, eur, 0.9167).setId(0);
        exchangeRates.add(usdEur);

        ExchangeModel eurUsd = new ExchangeModel(eur, usd, 1.0908).setId(1);
        exchangeRates.add(eurUsd);

        ExchangeModel rubEur = new ExchangeModel(rub, eur, 0.0104).setId(2);
        exchangeRates.add(rubEur);

        ExchangeModel eurRub = new ExchangeModel(eur, rub, 95.8475).setId(3);
        exchangeRates.add(eurRub);

        ExchangeModel usdRub = new ExchangeModel(usd, rub, 87.8644).setId(4);
        exchangeRates.add(usdRub);

        ExchangeModel rubUsd = new ExchangeModel(rub, usd, 0.0114).setId(5);
        exchangeRates.add(rubUsd);

        ExchangeModel jpyChy = new ExchangeModel(jpy, cny, 0.0460).setId(6);
        exchangeRates.add(jpyChy);

        ExchangeModel cnyJpy = new ExchangeModel(cny, jpy, 21.7568).setId(7);
        exchangeRates.add(cnyJpy);

        ExchangeModel usdJpy = new ExchangeModel(usd, jpy, 157.9515).setId(8);
